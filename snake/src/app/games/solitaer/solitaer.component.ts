...
export class SolitaerComponent {
  ...
  goal: Field[][] = [[new Field(Type.Null, Group.Clubs, Color.Black)], [new Field(Type.Null, Group.Spades, Color.Black)], [new Field(Type.Null, Group.Diamonds, Color.Red)], [new Field(Type.Null, Group.Hearts, Color.Red)]]
  ...
  tryCard(x: number, y: number): void {
    ...
    if (card)
    {
      for (let i = 0; i <= 3; i++) {
        ...
        if (lastCard && lastCard.group === card.group) {
      ...
      if (!moved){
        for (let i = 0; i < 7; i++) {
          ...
          console.log(topCard && topCard.type - 1 === card.type && topCard.color !== card.color);
          if (
